...
public class HalfCheckRecordOrderService extends BaseService {
	...
	public Pager getList(Pager pager, Date start_time, Date end_time,
		...
		try {
			...
			String seq = "AND ";
			if (companyId != null) {
				sql.append("select  a.*,c.end_at as end_at, c.name as name,c.companyId as companyId,c.charge_employee as charge_employee,c.orderNumber as orderNumber,  b.detail_json as detail_json  from tb_halfcheckrecordorder a,tb_planorder b,tb_order c where a.orderId=b.orderId and a.orderId=c.id and c.companyId='"
						+ companyId + "'");
				seq = " AND ";
			} else {
				sql.append("select  a.*,c.end_at as end_at,c.name as name,c.companyId as companyId,c.charge_employee as charge_employee,c.orderNumber as orderNumber,  b.detail_json as detail_json  from tb_halfcheckrecordorder a,tb_planorder b,tb_order c where a.orderId=b.orderId and a.orderId=c.id ");
			}
