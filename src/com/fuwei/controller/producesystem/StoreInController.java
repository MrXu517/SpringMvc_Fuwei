...
public class StoreInController extends BaseController {
	...
	public Map<String, Object> delete(@PathVariable int id,
		...
		StoreInOut storeIn = storeInOutService.get(id, true);
		//若原材料入库单已打印或已执行完成(即不可正常删除)，则执行数据纠正，否则正常执行删除
		Map<String,Object> data = new HashMap<String, Object>();
		if(!storeIn.deletable()){
			//判断是否有数据纠正的权限
			if(!hasAuthority_datacorrect){
				throw new PermissionDeniedDataAccessException("原材料入库单已打印或已执行完成，且没有数据纠正的权限，无法删除", null);
			}
			...
			dataCorrectRecord.setTb_table("原材料入库单");
			dataCorrectRecord.setDescription("原材料入库单" + storeIn.getNumber()+"已打印或已执行完成，因数据错误进行数据纠正删除");
			storeInOutService.remove_datacorrect(storeIn,dataCorrectRecord);
			data.put("message", "原材料入库单" + storeIn.getNumber() + " 数据纠正删除操作成功");
		}else{
			if (!hasAuthority) {
				throw new PermissionDeniedDataAccessException("没有删除原材料入库单的权限", null);
			}
	...
	@ResponseBody
	public ModelAndView addbyorder2(String coloringOrderNumber, HttpSession session,
		...
		//判断该染色单是大货纱 还是 样纱
		if(coloringOrder.getOrderId() == null){//样纱
			return addbyorder_coloring(coloringOrder.getId(), session, request, response);
		}else{//大货纱
			return addbyorder(coloringOrder.getId(), session, request, response);
		}
		
	}

	
	@RequestMapping(value = "/add/{coloringOrderId}", method = RequestMethod.GET)
	@ResponseBody
	public ModelAndView addbyorder(@PathVariable Integer coloringOrderId,
			HttpSession session, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		if(coloringOrder.getOrderId() == null){//样纱
			throw new Exception("该染色单是样纱染色单，请在样纱扫描处扫描");
		}
		StoreOrder storeOrder = storeOrderService.getByOrder(coloringOrder.getOrderId());
		if(storeOrder == null){
			throw new Exception("该原材料仓库单不存在或已被删除");
		}
		int storeOrderId = storeOrder.getId();
		...
		try {
			...
			request.setAttribute("storeOrder", storeOrder);
			request.setAttribute("factoryId", coloringOrder.getFactoryId());
	...
	public ModelAndView update(@PathVariable Integer id, HttpSession session,
		...
		try {
			if (id != null) {
				...
				if(storeInOut.getOrderId()!=null){//若是大货纱入库
