$(document).ready(function(){
	/*设置当前选中的页*/
	var $a = $("#left li a[href='selfaccount/index']");
	...
	$(".delete").click(function(){
		if(!confirm("您确定要删除该收支帐号吗？")){
			return false;
		}
		var id= $(this).attr("data-cid");
		$.ajax({
            url: "selfaccount/delete/"+id,
            type: 'POST'
        })
...
function setAdd(){
	...
	$submitBtn.text("添加帐号");
	$("#bank .formwidget .panel-title").text("添加本厂收支帐号");
	$form.unbind("submit");
...
function setUpdate(result){
	...
	var $submitBtn = $form.find("[type='submit']");
	$submitBtn.text("修改帐号");
	$("#bank .formwidget .panel-title").text("修改本厂收支账户");
