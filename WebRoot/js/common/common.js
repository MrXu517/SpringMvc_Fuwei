...
$(document).ajaxSend(function(event, xhr, settings) {
    ...
    if (settings.loading != false) {
        ...
        if (event.target.toString() == "[object HTMLDocument]" || event.target.toString() == "[object Document]") { //加载 公共加载框时，loadingNumbers+1
...
$(document).ajaxComplete(function(event, xhr, settings) {
    if (settings.loading != false) {
        if (event.target.toString() != "[object HTMLDocument]" & event.target.toString() != "[object Document]") { //某些有上下文引用的ajax请求也执行unload函数
...
Common.fillForm = function(formEle, data) {
    var inputs = $(formEle).find("input[type!='button'][type!='submit'],select,textarea");
...
Common.checkform = function(formEle) {
    var requireles = $(formEle).find("input.require,select.require,textarea.require");
    //验证必须填的字段
    for (var i = 0; i < requireles.length; ++i) {
        var requirele = requireles[i];
        if (!Common.requireCheck(requirele, requirele.value)) {
            var requirele_siblingstext = $(requirele).siblings("input[type='text']");
...
Common.intCheck_Rewrite = function(ele, value) {
    ...
    if (!myRegExp1.test(value) & !myRegExp2.test(value) & !myRegExp3.test(value)) {
...
function _c9_autoheight(){
    //设置表格的自适应高度
    $(".auto_height").each(function(index, el) {
        ...
        var height = $container.height() + $container.offset().top - $(this).offset().top - bottom_height;
        //height为自适应DOM的outerHeight(true)的高度，而非内容高度，因此还需要减去margin.border等
        var maring_border_height = $(this).outerHeight(true) - $(this).height();
