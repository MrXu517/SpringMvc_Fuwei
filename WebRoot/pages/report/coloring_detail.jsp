...
<%
	...
	List<ColoringOrder> result = (List<ColoringOrder>)request.getAttribute("result");
...
<html>
						...
						<div class="row">
							<div class="col-md-12 tablewidget">
								...
								<div clas="navbar navbar-default">
									...
									<a target="_blank" href="report/coloring_detail/export?factoryId=<%=factory_str %>&start_time=<%=start_time_str %>&end_time=<%=end_time_str %>" class="btn btn-primary">导出</a>
								</div>
								...
								<table class="table table-responsive">
									...
									<tbody>
										<%
											int i = 0 ;
											for (ColoringOrder order : result) {
												int count = 0 ;
												for (ColoringOrderDetail item : order.getDetaillist()) {
										%>
										<tr>
											<td><%=++i%></td>
											<%if(count==0){ %>
											<td><%=DateTool.formatDateYMD(order.getCreated_at())%></td>
											<td><%=SystemCache.getFactoryName(order.getFactoryId())%></td>
	...
	/*设置当前选中的页*/
	var $a = $("#left li a[href='report/coloring_detail']");
