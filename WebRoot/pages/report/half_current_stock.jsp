...
<%@page import="com.fuwei.entity.producesystem.HalfCurrentStock"%>
...
<%@page import="com.fuwei.entity.producesystem.HalfCurrentStockDetail"%>
<%
	...
	List<HalfCurrentStock> currentStocklist = new ArrayList<HalfCurrentStock>();
	if (pager != null & pager.getResult() != null) {
		currentStocklist = (List<HalfCurrentStock>) pager.getResult();
	}
							...
							<div class="col-md-12 tablewidget">
								...
								<div clas="navbar navbar-default">
									...
									<ul class="pagination">
										<li>
											<a
												href="half_current_stock/report?orderNumber=<%=orderNumber_str%>&charge_employee=<%=charge_employee_str%>&companyId=<%=company_str%>&page=1">«</a>
										</li>
										...
										<li class="">
											<a
												href="half_current_stock/report?orderNumber=<%=orderNumber_str%>&charge_employee=<%=charge_employee_str%>&companyId=<%=company_str%>&page=<%=pager.getPageNo() - 1%>">上一页
										...
										<li class="active">
											<a
												href="half_current_stock/report?orderNumber=<%=orderNumber_str%>&charge_employee=<%=charge_employee_str%>&companyId=<%=company_str%>&page=<%=pager.getPageNo()%>"><%=pager.getPageNo()%>/<%=pager.getTotalPage()%>，共<%=pager.getTotalCount()%>条<span
										...
										<li class="">
											<a
												href="half_current_stock/report?orderNumber=<%=orderNumber_str%>&charge_employee=<%=charge_employee_str%>&companyId=<%=company_str%>&page=<%=pager.getPageNo() + 1%>">下一页
												<span class="sr-only"></span> </a>
										</li>
										<%
											} else {
										...
										<li>
											<a
												href="half_current_stock/report?orderNumber=<%=orderNumber_str%>&charge_employee=<%=charge_employee_str%>&companyId=<%=company_str%>&page=<%=pager.getTotalPage()%>">»</a>
										</li>
									</ul>
									...
									<thead>
										...
										<tr>
											...
											<th rowspan="2" width="40px">
												...
												总库存
											</th><th colspan="3" width="165px">颜色及数量库存列表</th>
									...
									<tbody>
										<%
											int i = (pager.getPageNo()-1) * pager.getPageSize() + 0;
																											for (HalfCurrentStock item : currentStocklist) {
																												boolean even = i%2 == 0;
																												String classname = even?"even":"odd";
																												List<HalfCurrentStockDetail> detailist = item.getDetaillist();
										...
										<tr itemId="<%=item.getId()%>" class="<%=classname%>">
											...
											<td rowspan="<%=detailsize%>">
												<a target="_top" href="half_current_stock/in_out/<%=item.getOrderId()%>">出入库记录</a>
											</td>
										</tr>
										<%
											detailist.remove(0);
																		for(HalfCurrentStockDetail detail : detailist){
										%>
										<tr class="<%=classname %>">
