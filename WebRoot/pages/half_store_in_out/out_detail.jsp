				...
				<div class="row">
					<div class="col-md-12 tablewidget">
						...
						<%
							if(has_edit){
						%>
						<a href="half_store_out/put/<%=storeInOut.getId()%>" type="button" class="btn btn-primary">编辑</a>
						<%
							}
						%>
						<%
						...
						<table class="table noborder">
							<caption id="tablename">
								桐庐富伟针织厂半成品出库单<div table_id="<%=storeInOut.getId()%>" class="id_barcode"></div>
							</caption>
						</table>
	...
	<script type="text/javascript">
		...
		$("#deleteBtn").click( function() {
			...
			$.ajax( {
				...
				if (result.success) {
					Common.Tip("删除半成品出库单成功", function() {
						$("#breadcrumbs li.active").prev().find("a").click();
					});
				}
			}).fail( function(result) {
				Common.Error("删除半成品出库单失败：" + result.responseText);
			}).always( function() {
