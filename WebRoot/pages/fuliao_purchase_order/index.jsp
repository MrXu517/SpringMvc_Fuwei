...
<%
	...
	if (pager != null & pager.getResult() != null) {
		fuliaoPurchaseOrderlist = (List<FuliaoPurchaseOrder>) pager.getResult();
	}
...
.table>thead>tr>th,.table>tbody>tr>td{
							...
							<div class="col-md-12 tablewidget">
								...
								<div clas="navbar navbar-default">
									...
									<ul class="pagination">
										<li>
											<a
												href="fuliao_purchase_order/index?number=<%=number %>&factoryId=<%=factory_str %>&status=<%=status_str %>&companyId=<%=company_str %>&start_time=<%=start_time_str %>&end_time=<%=end_time_str %>&page=1">«</a>
										</li>
									...
									%>
										<li class="">
											<a
												href="fuliao_purchase_order/index?number=<%=number %>&factoryId=<%=factory_str %>&status=<%=status_str %>&companyId=<%=company_str %>&start_time=<%=start_time_str %>&end_time=<%=end_time_str %>&page=<%=pager.getPageNo() - 1%>">上一页
									...
									%>
										...
										<li class="active">
											<a
												href="fuliao_purchase_order/index?number=<%=number %>&factoryId=<%=factory_str %>&status=<%=status_str %>&companyId=<%=company_str %>&start_time=<%=start_time_str %>&end_time=<%=end_time_str %>&page=<%=pager.getPageNo() %>"><%=pager.getPageNo()%>/<%=pager.getTotalPage()%>，共<%=pager.getTotalCount()%>条<span
										...
										<li class="">
											<a
												href="fuliao_purchase_order/index?number=<%=number %>&factoryId=<%=factory_str %>&status=<%=status_str %>&companyId=<%=company_str %>&start_time=<%=start_time_str %>&end_time=<%=end_time_str %>&page=<%=pager.getPageNo() + 1%>">下一页
												<span class="sr-only"></span> </a>
										</li>
										<%
										} else {
									...
									%>
										...
										<li>
											<a
												href="fuliao_purchase_order/index?number=<%=number %>&factoryId=<%=factory_str %>&status=<%=status_str %>&companyId=<%=company_str %>&start_time=<%=start_time_str %>&end_time=<%=end_time_str %>&page=<%=pager.getTotalPage()%>">»</a>
										</li>
									</ul>
									...
									<tbody>
										<%
											...
											for (FuliaoPurchaseOrder fuliaoPurchaseOrder : fuliaoPurchaseOrderlist) {
												List<FuliaoPurchaseOrderDetail> detailist = fuliaoPurchaseOrder.getDetaillist();
												int detailsize = fuliaoPurchaseOrder.getDetaillist().size();
										%>
										<tr orderId="<%=fuliaoPurchaseOrder.getId()%>">
											<td rowspan="<%=detailsize %>"><%=++i%></td>
											<td rowspan="<%=detailsize %>"><%=fuliaoPurchaseOrder.getNumber() == null ? "":fuliaoPurchaseOrder.getNumber() %></td>
											<td rowspan="<%=detailsize %>"><%=SystemCache.getFactoryName(fuliaoPurchaseOrder.getFactoryId())%></td>
											<td rowspan="<%=detailsize %>"><%=SystemCache.getCompanyShortName(fuliaoPurchaseOrder
										.getCompanyId())%></td>
											<td rowspan="<%=detailsize %>"><%=SystemCache.getCustomerName(fuliaoPurchaseOrder.getCustomerId())%></td>
											<td rowspan="<%=detailsize %>"><%=fuliaoPurchaseOrder.getCompany_productNumber()%></td>
											...
											<td rowspan="<%=detailsize %>"><%=SystemCache.getUserName(fuliaoPurchaseOrder
										.getCreated_user())%></td>
											<td rowspan="<%=detailsize %>"><%=DateTool.formatDateYMD(fuliaoPurchaseOrder.getCreated_at())%></td>
											<td rowspan="<%=detailsize %>">
												<%
													if (has_order_detail) {
												%>
												<a target="_blank"
													href="fuliao_purchase_order/detail/<%=fuliaoPurchaseOrder.getId()%>">详情</a>
												<%
													}
												%>
												<%
												
													if (has_order_edit && fuliaoPurchaseOrder.isEdit()) {
												%>
												|
												<a
													href="fuliao_purchase_order/put/<%=fuliaoPurchaseOrder.getId()%>">编辑</a>
												<%
														}
													%>
												<%
												
													if (has_order_delete && fuliaoPurchaseOrder.deletable()) {
												%>
												|
												<a href="#" data-cid="<%=fuliaoPurchaseOrder.getId() %>"
													class="delete">删除</a>
												<%
														}
													%>
	...
	$(".delete").click( function() {
		...
		$.ajax( {
			url :"fuliao_purchase_order/delete/" + id,
			type :'POST'
