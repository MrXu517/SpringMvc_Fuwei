...
<%
	...
	List<FuliaoInNotice> resultlist = (List<FuliaoInNotice>)request.getAttribute("resultlist");
...
<html>
	<head>
		<base href="<%=basePath%>">
		<title>订单辅料入库通知单列表 -- 桐庐富伟针织厂</title>
		<meta charset="utf-8">
		...
		<script src="js/fuliaoin_notice/listbyorder.js" type="text/javascript"></script>
		...
		<div id="Content">
			<div id="main">
				<div class="body">
								...
								<%if(has_add){ %>
									<a target="_blank" id="addBtn" href="fuliaoin_notice/add/<%=orderId %>" class="btn btn-primary" >创建入库通知单</a>
								<%} %>
								</div>
								...
								<table class="table table-responsive table-bordered" id="Tb">
									...
									<tbody>
										...
										<tr><td colspan="12">还未添加入库通知单，请点击按钮 "创建入库通知单" 添加</td></tr>
										<%} %>
										<%
											
											for (FuliaoInNotice notice : resultlist) {
										%>
										...
										<tr itemId="<%=notice.getId()%>">
											...
											<td>
												<a target="_blank" href="fuliaoin_notice/detail/<%=notice.getId()%>">详情</a>
												<%
													if (has_edit) {
												%>
												<a target="_blank" href="fuliaoin_notice/put/<%=notice.getId()%>">编辑</a>
												<%
													}
												%>
												<%
