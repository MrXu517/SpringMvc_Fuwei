...
<html>
	<head>
		<base href="<%=basePath%>">
		<title>通用辅料入库通知单详情 -- 桐庐富伟针织厂</title>
		<meta charset="utf-8">
						...
						<div class="row">
							<div class="col-md-12">
								<%
									if(has_print){
								%>
								<a target="_blank" href="fuliaoin_notice/print/<%=notice.getId()%>" type="button" class="btn btn-success">打印</a>
								<%
									}
								%>
								<%
									if(has_edit){
								%>
								<a target="_blank" href="fuliaoin_notice/put/<%=notice.getId()%>" type="button" class="btn btn-success">编辑</a>
								<%
									}
								%>
								<form class="saveform">
									...
									<div class="col-md-12 tablewidget">
										<table class="table">
											<caption id="tablename">
												桐庐富伟针织厂通用辅料入库通知单<div table_id="<%=notice.getNumber()%>" class="id_barcode"></div>
											<%
										...
										<table id="mainTb"
											...
											<tbody>
												...
												%>
												<tr class="tr">
													<td><%=SystemCache.getFuliaoTypeName((Integer)detail.getFuliaoTypeId())%>
													<br><%=detail.getFnumber()%>
													</td>
