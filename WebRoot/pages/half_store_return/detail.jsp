...
<%
	...
	+ path + "/";
	//半成品退货单
	HalfStoreReturn storeReturn = (HalfStoreReturn) request.getAttribute("storeReturn");
...
<%@ include file="../common/head.jsp"%>
				...
				<div class="row">
					<div class="col-md-12 tablewidget">
						<%
							if(has_print){
						%>
						<a target="_blank" href="half_store_return/print/<%=storeReturn.getId()%>" type="button" class="btn btn-success">打印</a>
						<%
							}
						%>
	...
	<script type="text/javascript">
		...
		$("#deleteBtn").click( function() {
			...
			if (!confirm("确定要删除该半成品退货单吗？")) {
				return false;
			}
			$.ajax( {
				url :"half_store_return/delete/" + id,
				type :'POST'
			}).done( function(result) {
				if (result.success) {
					Common.Tip("删除半成品退货单成功", function() {
						$("#breadcrumbs li.active").prev().find("a").click();
					});
				}
			}).fail( function(result) {
				Common.Error("删除半成品退货单失败：" + result.responseText);
			}).always( function() {
