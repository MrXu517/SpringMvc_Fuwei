...
<html>
	<head>
		...
		<script src="js/packing_order/edit.js" type="text/javascript"></script>
		<style type="text/css">
						...
						<div class="row">
							<form class="saveform">
								<input type="hidden" id="id" name="id"
									value="<%=packingOrder.getId()%>" class="require" />
								<button type="submit"
												...
												<td width="60%">
													<div class="form-group" style="width: auto;">
														备注：<input style="width: 350px;" type="text" class="form-control"
																			name="memo" id="memo" value="<%=packingOrder.getMemo()==null?"":packingOrder.getMemo() %>">
													</div>
												</td>
											</tr>
										</tbody>
									...
									<table>
										...
										<tr>
											<th rowspan="2" width="80px">
												...
												<input type="checkbox" class="colable" checked>
												<%}else{ %>
												...
												<%for(int i = 0 ; i < propertylist.size();++i){ 
													...
													if(packingOrder.getCol1_id()!=null && packingOrder.getCol1_id() == item.getId()){
												%>
											...
											<th rowspan="2" width="80px">
												...
												<%for(int i = 0 ; i < propertylist.size();++i){ 
													PackProperty item = propertylist.get(i);
													if(packingOrder.getCol2_id()!=null && packingOrder.getCol2_id() == item.getId()){
												%>
												<option selected value="<%=item.getId() %>"><%=item.getName() %></option>
												<%}else{ %>
													<option value="<%=item.getId() %>"><%=item.getName() %></option>
											...
											<th rowspan="2" width="80px">
												...
												<%for(int i = 0 ; i < propertylist.size();++i){ 
													PackProperty item = propertylist.get(i);
													if(packingOrder.getCol3_id()!=null && packingOrder.getCol3_id() == item.getId()){
												%>
												<option selected value="<%=item.getId() %>"><%=item.getName() %></option>
												<%}else{ %>
													<option value="<%=item.getId() %>"><%=item.getName() %></option>
											...
											<th rowspan="2" width="80px">
												...
												<%for(int i = 0 ; i < propertylist.size();++i){ 
													PackProperty item = propertylist.get(i);
													if(packingOrder.getCol4_id()!=null && packingOrder.getCol4_id() == item.getId()){
												%>
												<option selected value="<%=item.getId() %>"><%=item.getName() %></option>
												<%}else{ %>
													<option value="<%=item.getId() %>"><%=item.getName() %></option>
