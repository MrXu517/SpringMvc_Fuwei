...
<html>
	<head>
		<base href="<%=basePath%>">
		<title>收支明细 -- 财务工作台 -- 桐庐富伟针织厂</title>
		<meta charset="utf-8">
				...
				<div class="body">
					<div id="tab">
						...
						<div class="tab-content">
										...
										<div class="col-md-12 tablewidget">
											...
											<div clas="navbar navbar-default">
												...
												<ul class="pagination">
													<li>
														<a
															href="order/index?bank_id=<%=bank_str%>&subject_id=<%=subject_str%>&amount_from=<%=amount_from_str%>&in_out=<%=in_out_str%>&amount_to=<%=amount_to_str%>&companyId=<%=company_str%>&salesmanId=<%=salesman_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=1">«</a>
													</li>
													...
													<li class="">
														<a
															href="order/index?bank_id=<%=bank_str%>&subject_id=<%=subject_str%>&amount_from=<%=amount_from_str%>&in_out=<%=in_out_str%>&amount_to=<%=amount_to_str%>&companyId=<%=company_str%>&salesmanId=<%=salesman_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getPageNo() - 1%>">上一页
													...
													<li class="">
														<a
															href="order/index?bank_id=<%=bank_str%>&subject_id=<%=subject_str%>&amount_from=<%=amount_from_str%>&in_out=<%=in_out_str%>&amount_to=<%=amount_to_str%>&companyId=<%=company_str%>&salesmanId=<%=salesman_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getPageNo() + 1%>">下一页
															<span class="sr-only"></span> </a>
													</li>
													<%
														} else {
													...
													<li>
														<a
															href="order/index?bank_id=<%=bank_str%>&subject_id=<%=subject_str%>&amount_from=<%=amount_from_str%>&in_out=<%=in_out_str%>&amount_to=<%=amount_to_str%>&companyId=<%=company_str%>&salesmanId=<%=salesman_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getTotalPage()%>">»</a>
													</li>
												</ul>
										...
										.getCompany_id())%></td>
														...
														<td>
															<%
																if (item.getIn_out()) {
															%><a
																href="income/detail/<%=item.getId()%>">详情</a>
															<%
																} else {
															%><a href="expense/detail/<%=item.getId()%>">详情</a>
															<%
																}
															%>
							...
							<div class="tab-pane" id="purchase_invoices"></div>
						</div>
					</div>
