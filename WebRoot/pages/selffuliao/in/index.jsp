...
<%
	...
	List<SelfFuliaoIn> inOutlist = new ArrayList<SelfFuliaoIn>();
	if (pager != null & pager.getResult() != null) {
		inOutlist = (List<SelfFuliaoIn>) pager.getResult();
	}
							...
							<div class="col-md-12 tablewidget">
								...
								<div clas="navbar navbar-default">
									...
									<ul class="pagination">
										<li>
											<a
												href="selffuliaoin/index?number=<%=number_str%>&charge_employee=<%=charge_employee_str%>&orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=1">«</a>
										</li>
										...
										<li class="">
											<a
												href="selffuliaoin/index?number=<%=number_str%>&charge_employee=<%=charge_employee_str%>&orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getPageNo() - 1%>">上一页
										...
										<li class="active">
											<a
												href="selffuliaoin/index?number=<%=number_str%>&charge_employee=<%=charge_employee_str%>&orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getPageNo()%>"><%=pager.getPageNo()%>/<%=pager.getTotalPage()%>，共<%=pager.getTotalCount()%>条<span
										...
										<li class="">
											<a
												href="selffuliaoin/index?number=<%=number_str%>&charge_employee=<%=charge_employee_str%>&orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getPageNo() + 1%>">下一页
												<span class="sr-only"></span> </a>
										</li>
										<%
											} else {
										...
										<li>
											<a
												href="selffuliaoin/index?number=<%=number_str%>&charge_employee=<%=charge_employee_str%>&orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getTotalPage()%>">»</a>
										</li>
									</ul>
									...
									<thead>
										...
										<tr>
											...
											<th rowspan="2" width="40px">
												跟单人
											</th><th colspan="4" width="165px">自购辅料入库列表</th>
											<th rowspan="2" width="60px">
												入库时间
											</th>
									...
									<tbody>
										<%
											int i = (pager.getPageNo()-1) * pager.getPageSize() + 0;
																											for (SelfFuliaoIn item : inOutlist) {
																												boolean even = i%2 == 0;
																												String classname = even?"even":"odd";
																												List<SelfFuliaoInDetail> detailist = item.getDetaillist();
																												if(detailist == null){detailist = new ArrayList<SelfFuliaoInDetail>();}
																												int detailsize = detailist.size();
										%>
										<tr itemId="<%=item.getId()%>" class="<%=classname%>">
											<td rowspan="<%=detailsize%>"><%=++i%></td>
											
											<td rowspan="<%=detailsize%>"><a target="_blank" href="selffuliaoin/detail/<%=item.getId()%>"><%=item.getNumber()%></a></td>
											...
											<td rowspan="<%=detailsize%>">
												<a target="_blank" href="selffuliaoin/detail/<%=item.getId()%>">详情</a>
												
											</td>
										</tr>
										<%
											detailist.remove(0);
											for(SelfFuliaoInDetail detail : detailist){
										%>
										<tr class="<%=classname %>">
