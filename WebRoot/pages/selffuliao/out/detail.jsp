...
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ path + "/";
	SelfFuliaoOut object = (SelfFuliaoOut)request.getAttribute("selfFuliaoOut");
	List<SelfFuliaoOutDetail> detaillist = object.getDetaillist();
	if (detaillist == null) {
...
#created_user{  margin-right: 50px;}
						...
						<div class="row">
							<div class="col-md-12">
								<%
									if(has_print){
								%>
								<a target="_blank" href="selffuliaoout/print/<%=object.getId()%>" type="button" class="btn btn-success">打印</a>
								<a target="_blank" href="selffuliaoout/print/<%=object.getId() %>/tag" type="button" class="btn btn-success">打印辅料标签</a>
								<%
									}
								%>
								...
								<form class="saveform">
									...
									<div class="col-md-12 tablewidget">
										<table class="table">
											<caption id="tablename">
												桐庐富伟针织厂自购辅料出库单<div table_id="<%=object.getNumber()%>" class="id_barcode"></div>
											</caption>
										</table>
									...
									<td>
										领取人：
										<span><%=SystemCache.getEmployeeName(object.getReceiver_employee())%></span>
									</td>
									<td class="pull-right">
	...
	<script type="text/javascript">
		...
		$("#deleteBtn_datacorrect").click( function() {
			...
			if (!confirm("该自购辅料出库单已打印出库，请确保辅料实际未出库再进行删除操作， 您是否确定要进行数据纠正：删除？")) {
				return false;
			}
			$.ajax( {
				url :"selffuliaoout/delete/" + id,
				type :'POST'
