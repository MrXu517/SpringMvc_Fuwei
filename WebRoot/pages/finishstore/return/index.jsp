...
<%
	...
	List<FinishStoreReturn> inOutlist = new ArrayList<FinishStoreReturn>();
	if (pager != null & pager.getResult() != null) {
		inOutlist = (List<FinishStoreReturn>) pager.getResult();
	}
							...
							<div class="col-md-12 tablewidget">
								...
								<div clas="navbar navbar-default">
									...
									<ul class="pagination">
										<li>
											<a
												href="finishstore_return/index?orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=1">«</a>
										</li>
										...
										<li class="">
											<a
												href="finishstore_return/index?orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getPageNo() - 1%>">上一页
										...
										<li class="active">
											<a
												href="finishstore_return/index?orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getPageNo()%>"><%=pager.getPageNo()%>/<%=pager.getTotalPage()%>，共<%=pager.getTotalCount()%>条<span
										...
										<li class="">
											<a
												href="finishstore_return/index?orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getPageNo() + 1%>">下一页
												<span class="sr-only"></span> </a>
										</li>
										<%
											} else {
										...
										<li>
											<a
												href="finishstore_return/index?orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getTotalPage()%>">»</a>
										</li>
									</ul>
								...
								<table class="table table-responsive table-bordered">
									<thead>
										...
										<tr>
											...
											<th rowspan="2" width="40px">
												跟单人
											</th><th colspan="7" width="165px">退货列表</th>
											<th rowspan="2" width="60px">
												退货时间
											</th>
											<th rowspan="2" width="40px">
									...
									<tbody>
										<%
											...
											for (FinishStoreReturn item : inOutlist) {
												boolean even = i%2 == 0;
												String classname = even?"even":"odd";
												List<FinishStoreReturnDetail> detailist = item.getDetaillist();
												if(detailist == null){detailist = new ArrayList<FinishStoreReturnDetail>();}
										...
										<tr itemId="<%=item.getId()%>" class="<%=classname%>">
											...
											<td rowspan="<%=detailsize%>">
												<a target="_blank" href="finishstore_return/detail/<%=item.getId()%>">详情</a>
												
											</td>
										</tr>
										<%
											detailist.remove(0);
											for(FinishStoreReturnDetail detail : detailist){
										%>
										<tr class="<%=classname %>">
