...
<%
	...
	List<FinishStoreIn> inOutlist = new ArrayList<FinishStoreIn>();
	if (pager != null & pager.getResult() != null) {
		inOutlist = (List<FinishStoreIn>) pager.getResult();
	}
							...
							<div class="col-md-12 tablewidget">
								...
								<div clas="navbar navbar-default">
									...
									<ul class="pagination">
										<li>
											<a
												href="finishstore_in/index?orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=1">«</a>
										</li>
										...
										<li class="">
											<a
												href="finishstore_in/index?orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getPageNo() - 1%>">上一页
										...
										<li class="active">
											<a
												href="finishstore_in/index?orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getPageNo()%>"><%=pager.getPageNo()%>/<%=pager.getTotalPage()%>，共<%=pager.getTotalCount()%>条<span
										...
										<li class="">
											<a
												href="finishstore_in/index?orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getPageNo() + 1%>">下一页
												<span class="sr-only"></span> </a>
										</li>
										<%
											} else {
										...
										<li>
											<a
												href="finishstore_in/index?orderNumber=<%=orderNumber_str%>&start_time=<%=start_time_str%>&end_time=<%=end_time_str%>&page=<%=pager.getTotalPage()%>">»</a>
										</li>
									</ul>
								...
								<table class="table table-responsive table-bordered">
									...
									<tbody>
										<%
											...
											for (FinishStoreIn item : inOutlist) {
												boolean even = i%2 == 0;
												String classname = even?"even":"odd";
												List<FinishStoreInDetail> detailist = item.getDetaillist();
												if(detailist == null){detailist = new ArrayList<FinishStoreInDetail>();}
										...
										<tr itemId="<%=item.getId()%>" class="<%=classname%>">
											...
											<td rowspan="<%=detailsize%>">
												<a target="_blank" href="finishstore_in/detail/<%=item.getId()%>">详情</a>
												
											</td>
										</tr>
										<%
											detailist.remove(0);
											for(FinishStoreInDetail detail : detailist){
										%>
										<tr class="<%=classname %>">
